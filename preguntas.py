...
def pregunta_01():
    ...
    df = pd.read_csv(
        ...
        sep='\t',
        header=None,
        names=["Comentario", "Etiquetado"],
    )

    # Separe los grupos de mensajes etiquetados y no etiquetados.
    df_tagged = df[df["Etiquetado"].notnull()]
    df_untagged =df[df["Etiquetado"].isnull()]

    x_tagged = df_tagged["Comentario"]
    y_tagged = df_tagged["Etiquetado"]

    x_untagged = df_untagged["Comentario"]
    y_untagged = df_untagged["Etiquetado"]
